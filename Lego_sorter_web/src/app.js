...
async function initAPP() 
{
  ...
  for (let i = 0; i < bricks.length; i++) {
    ...
    btn.innerHTML =
      '<img src="./images/' + bricks[i] + '" width="80" height ="80" />';
...
async function getModelInference(rgbPath,topk = 3, debug = true) {
    try {
      ...
      let sortedResult = prediction2Array(modelResults.brick_type.data);
      
      const brick_id_to_name = {
            0 : "6*2",
            1 : "3*1",
            2 : "3*2",
            3 : "2*1",
...
function printResults(url, results, dims) {
...
async function getImageTensorFromPath(rgbPath, dims =  [1, 3, 256, 256]) 
{
	// 1. load the image
